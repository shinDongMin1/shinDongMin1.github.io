...
클라이언트 -> API Gateway -> Lambda 함수 -> Aurora PostgreSQL.
Lambda 할당량을 대폭 늘림.
...
온프레미스 MySQL 데이터베이스를 Amazon Web Services(AWS)로 마이그레이션.
엄청난 양의 쓰기 작업이 발생.
...
각 대상 그룹은 지정한 TCP 프로토콜과 포트 번호를 사용하여 EC2 인스턴스 같은 하나 이상의 등록된 대상으로 요청을 라우팅.
여러 대상 그룹에 대상을 등록할 수 있습니다. 대상 그룹 기준으로 상태 확인을 구성.
대상에서 상태 검사가 수행.
...
온프레미스 데이터 센터에서 Oracle 관계형 데이터베이스를 사용하여 웹 애플리케이션을 운영.
데이터베이스를 AWS로 마이그레이션.
...
구성 파일에 Amazon RDS MySQL DB 인스턴스의 사용자 이름과 암호를 보관.
Amazon EC2 인스턴스 루트 디바이스 디스크에 일반 텍스트로 저장.
...
VPC 내부에 포함된 Amazon EC2 인스턴스에서 작동.
저장하고 검색하려면 앱 중 하나가 Amazon S3 API에 요청.

인스턴스 -> 인터페이스 엔드포인트 -> PrivateLink -> AWS 서비스 -> 인스턴스.
...
MySQL 데이터베이스는 기업의 주문 이행 서비스.
많은 양의 동시 요청 및 트랜잭션을 처리.
개발자에 의해 패치되고 조정하여 새로운 제품 기능의 도입이 지연.
